...
#else	
void _gasnetc_deregister_pd(gni_post_descriptor_t *pd)
#define gasnetc_deregister_pd(pd,didx) _gasnetc_deregister_pd(pd)
#endif
{
#if GNI_MULTI_DOMAIN
  gni_nic_handle_t nic_handle = gasnetc_cdom_data[didx].nic_handle;
...
void gasnetc_get_am_credit(uint32_t pe)
{
  ...
  if_pf (!gasnetc_weakatomic_dec_if_positive(p)) {
    ...
    do {
      GASNETI_WAITHOOK();
#if GNI_MULTI_DOMAIN
      gasnetc_poll(didx); /* this should converge faster, but doesn't progress PSHM */
#else 
      gasneti_AMPoll();
#endif
...
#else 
void _gasnetc_free_bounce_buffer(void *gcb)
{
