  ...
  if (gasnetc_bootstrap_am_coll) {
    ...
    for (i = 0; i < gasnetc_dissem_peers; ++i) { /* EMPTY for all but first per supernode */
      ...
      while (!(gasnetc_sys_barrier_rcvd[phase] & mask)) {
         gasnetc_poll(GASNETC_GNI_DEFAULT_DOMAIN);  /* No PSHM progress required here */
      }
    }
...
extern int gasnetc_attach(gasnet_handlerentry_t *table, int numentries,
  ...
  gasnetc_init_post_descriptor_pool(GASNETC_GNI_DEFAULT_DOMAIN);
  gasnetc_init_bounce_buffer_pool(GASNETC_GNI_DEFAULT_DOMAIN);
...
extern int gasnetc_AMRequestLongM( gasnet_node_t dest,        /* destination node */
...
#endif
  {
    ...
    if (!is_packed) {
      gasnetc_post_descriptor_t *gpdl = gasnetc_alloc_post_descriptor(GASNETC_GNI_DEFAULT_DOMAIN);
...
extern int gasnetc_AMReplyLongM( 
...
#endif
  {
    ...
    if (!is_packed) {
      ...
      gpdl->flags = GC_POST_COMPLETION_FLAG;
      gasneti_suspend_spinpollers();
      gasnetc_rdma_put_bulk(reply_node(token), dest_addr, source_addr, nbytes, gpdl);
      gasneti_resume_spinpollers();
    }
