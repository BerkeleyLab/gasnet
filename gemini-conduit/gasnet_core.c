...
static void gasnetc_sys_coll_init(void)
{
  ...
  { int step;
    ...
    gasnetex_rank_t last = (rank + size - (1<<(gasnetc_dissem_peers-1))) % size;
    ...
    for (step = i = 0; step < gasnetc_dissem_peers; ++step) {
      const gasnetex_rank_t distance = 1 << step;
...
extern int gasnetc_attach(gasnet_handlerentry_t *table, int numentries,
  ...
  { /*  core API handlers */
    ...
    if (gasneti_reghandlers(ctable, len, 1, GASNETE_HANDLER_BASE-1, 0, &numreg) != GASNET_OK)
  ...
  { /*  extended API handlers */
    ...
    if (gasneti_reghandlers(etable, len, GASNETE_HANDLER_BASE, 127, 0, &numreg) != GASNET_OK)
  ...
  { /* firehose handlers */
    ...
    if (gasneti_reghandlers(ftable, len, base, 127, 1, &numreg) != GASNET_OK)
...
int gasnetc_put_long_payload( gasnetex_rank_t dest,
                              void *dst_addr,
                              void *src_addr,
                              size_t nbytes,
                              gasneti_weakatomic_t *completed_p
...
int gasnetc_put_longasync_payload( gasnetex_rank_t dest,
                                   void *dst_addr,
                                   void *src_addr,
                                   size_t nbytes,
