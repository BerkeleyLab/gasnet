...
#define GASNET_COLL_TEAM_ROOT_BITS 16
...
gasnete_hashtable_t *gasnete_global_team_dir;

int gasnete_coll_smp_tune_barriers;

gasnet_hsl_t gasnete_coll_team_id_lock;
static uint32_t gasnete_coll_team_id_seq = 0;
...
#ifdef DEBUG_TEAM
  fprintf(stderr, "[%u]: gasnete_coll_team_td_init: team_id %d, team handle %p, my_image %u, my_local_image %u\n",
          td->my_image, team_id, team, my_image, my_local_image);
#endif
...
gasnet_team_handle_t gasnete_coll_team_lookup(uint32_t team_id)
{
  ...
  gasnet_team_handle_t team;
    ...
    if (gasnete_hashtable_search(gasnete_global_team_dir, team_id, (void **)&team)) {
...
{
  int i;
  fprintf(fp, "[%u] team handle %p,  team id %u, total ranks %u, my rank %u, total images %u, my image %u\n",
  ...
  fprintf(fp, "image_compact_order: ");
  for (i=0; i<team->total_images; i++) {
    fprintf(fp, "%u ", (unsigned int)team->image_compact_order[i]);
  }
  fprintf(fp, "\n");
                                   ...
                                   GASNETE_THREAD_FARG) 
{
  ...
  gasnete_hashtable_insert(gasnete_global_team_dir, team->team_id, team);
                               ...
                               GASNETE_THREAD_FARG)
{
  ...
  team->image_compact_order = (gasnet_image_t *)gasneti_malloc(sizeof(gasnet_image_t) * total_images);
  for (j=0; j<total_images; j++) {
    team->image_rel2act_map[j] = j;
    team->image_compact_order[j] = j;
  }
...
void gasnete_coll_team_fini(gasnet_team_handle_t team  GASNETE_THREAD_FARG)
{
  ...
  if (gasnete_coll_team_my_local_image(team GASNETE_THREAD_PASS) == 0) {
    gasnete_hashtable_remove(gasnete_global_team_dir, team->team_id, NULL);
  }

  /* remove team from the td->my_teams list */
  if (team_td == td->my_teams) {
    td->my_teams = team_td->next;
  } else {
    gasnete_coll_team_threaddata_t *team_tmp = td->my_teams;
    while (team_tmp->next != NULL && team_tmp->next != team_td) {
      team_tmp = team_tmp->next;
    }
    gasneti_assert(team_tmp->next == team_td);
    team_tmp->next = team_td->next;
  }
                                    ...
                                    void *clientdata)
{
  ...
  gasneti_assert(gasnet_AMGetMsgSource (token, &src) == GASNET_OK);
...
#ifdef DEBUG_TEAM
  fprintf(stderr, "gasnete_coll_teamid_reqh: src node %u, new_teamid %u, team_id_count %u\n", 
          src, teamid, (uint32_t)team_id_count);
  fflush(stderr);
#endif
                                              ...
                                              GASNETE_THREAD_FARG)
{
  ...
  uint32_t i, j, local_image_count;
  ...
  /* Only the first local thread in the process in the new team
     initializes the team data structure shared by other threads in
     the same process in the team. */
  if (gasnete_coll_team_my_image(parent_team) == first_local) {
    gasnet_image_t current_image;
    ...
    new_team->image_compact_order = (gasnet_image_t *)gasneti_malloc(total_images * sizeof(gasnet_image_t));
    ...
    for (i=0; i<total_images; i++) {
      ...
      new_team_members[i].done = 0;
      new_team->image_rel2act_map[i] = new_team_members[i].act_imageid;
    }
    
    current_rank = 0; 
    current_image = 0;
    for (i=0; i<total_images; i++) {
      if (new_team_members[i].done == 0) {
        new_team->image2rank_map[i] = current_rank;
        new_team->image_compact_order[current_image++] = i;
        local_image_count = 1;
        new_team_members[i].done = 1;

        for (j=i+1; j<total_images; j++) {
          if (new_team_members[j].done == 0 && 
              new_team_members[i].act_rank == new_team_members[j].act_rank) {
            new_team->image2rank_map[j] = current_rank; 
            new_team->image_compact_order[current_image++] = j;
            local_image_count++;
            new_team_members[j].done = 1;
          }
        }

        all_images[current_rank] = local_image_count;
        new_team->rel2act_map[current_rank] = new_team_members[i].act_rank;
        current_rank++;
      } /* end of if */
    }
    gasneti_assert(current_image == total_images);
...

    {
      gasnet_image_t j=0;
      for (i=0; i<total_images; i++) {
        if (new_team->image2rank_map[i] == myrank) {
          if (j >= all_images[myrank]) {
            gasneti_fatalerror("myrank %u, j %u > all_images[myrank] %u\n", myrank, j, all_images[myrank]);
          }
          new_team->local_images[j] = new_team_members[i].team_imageid;
          j++;
        }
      }
      gasneti_assert(j==all_images[myrank]);
    }

#ifdef DEBUG_TEAM
    fprintf(stderr, "[%u] myrank %u local images: ", 
            gasnete_coll_team_my_image(GASNET_TEAM_ALL), myrank);
    ...
    { 
      ...
      for (i=0; i<parent_team->my_images; i++) {
        ...
        if (gasnete_coll_team_find_image(images,
          ...
          if (current >= all_images[myrank]) {
            gasneti_fatalerror("myrank %u, current %u > all_images[myrank] %u\n", 
                               myrank, current, all_images[myrank]);
          }
  ...
  {
    smp_coll_t smp_coll_handle;
    gasnet_image_t my_local_image =  2147483647;

    /* Find out my local image order */
    for (i=0; i<new_team->my_images; i++) {
#if 0
      printf("[%u] new_team->local_images[%u]=%u\n", 
             gasnete_coll_team_my_image(GASNET_TEAM_ALL), 
             i, new_team->local_images[i]); 
#endif
    ...
    gasneti_assert(my_local_image != 2147483647);

    if (new_team->my_images > 1) { 
      smp_coll_handle = 
        smp_coll_team_init(1024*1024, SMP_COLL_SKIP_TUNE_BARRIERS,
                           new_team->my_images, my_local_image,
                           /* The team lead is the absolute local
                              image id of the first local image in the
                              new team, where the absolute local image
                              id of an image is its local rank in
                              GASNET_TEAM_ALL (td->my_local_image). */
                           (new_team->image_rel2act_map[new_team->local_images[0]] 
                            - GASNET_TEAM_ALL->my_offset)); 
    } else {
      smp_coll_handle = 
                                             ...
                                             GASNETE_THREAD_FARG)
{
  ...
  gasnet_image_t my_root_act_rank;
  uint32_t my_root_seq_num;

#ifdef DEBUG_TEAM
  fprintf(stderr, "[%u] gasnete_coll_team_split() begins: parent team handle %p, mycolor %u, my_image %u clientdata %p\n",
          gasnete_coll_team_my_image(GASNET_TEAM_ALL), parent_team, mycolor, 
          my_image, myscratch_seginfo);
  fflush(stderr);
#endif
  ...
  if (my_image == 0) {
    gasnet_hsl_lock(&gasnete_coll_team_id_lock);
    my_split_info.team_id_seq = gasnete_coll_team_id_seq++;
    gasnet_hsl_unlock(&gasnete_coll_team_id_lock);
  } else {
    my_split_info.team_id_seq = 0; /* shouldn't be actually used */
  }
  ...
  */
  ...
  fprintf(stderr, "[%u] gasnete_coll_team_split: after gather_all. \n",
          gasnete_coll_team_my_image(GASNET_TEAM_ALL));
  for (i=0; i<parent_team->total_images; i++) {
    fprintf(stderr, "[%u] all_split_info[%u].color = %d\n", 
            gasnete_coll_team_my_image(GASNET_TEAM_ALL), i,
            all_split_info[i].color);;
    fprintf(stderr, "[%u] all_split_info[%u].imageid = %d\n",
            gasnete_coll_team_my_image(GASNET_TEAM_ALL), i,
            all_split_info[i].imageid);
  }
#endif
  ...
  if (my_image != GASNET_IMAGE_UNDEFINED) {  
    ...
    for (i=0; i<parent_team->total_images; i++) {
      if (mycolor == all_split_info[i].color) {
        ...
        if (all_split_info[i].imageid == 0) {
          my_root_act_rank = parent_team->rel2act_map[parent_team->image2rank_map[i]];
          my_root_seq_num = all_split_info[i].team_id_seq;
          gasneti_assert(my_root_seq_num != 0);
        }
        new_total_images++;
      }
    }
    ...
    for (i=0; i<parent_team->total_images; i++) {
      ...
      if (gasnete_coll_team_my_image(parent_team) == i) {
        fprintf(stderr, "[%u] gasnete_coll_team_split: new_total_images %u, my_image %u, my_team_color %u, new_team_count %u.\n",
                gasnete_coll_team_my_image(GASNET_TEAM_ALL), new_total_images, 
                my_image, my_team_color_rank, new_team_count);
  ...
  new_team_id = my_root_act_rank << GASNET_COLL_TEAM_ROOT_BITS | my_root_seq_num;
