      ...
      if (op->team->myrank != args->srcnode) {
        ...
        gasnete_begin_nbi_accessregion(0,1 GASNETE_THREAD_PASS);
        {
          ...
          for (i = op->team->myrank + 1; i < op->team->total_ranks; ++i) {
            gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, i), dst, src, nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
          }
      ...
      if (op->team->myrank == args->srcnode) {
        ...
        for (child = 0; child < child_count; child++) {
          if(tree->geom->subtree_sizes[child] == 1) {
            ...
            /* Use NB Put + Sync */
            gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, children[child]), args->dst, args->src, args->nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
          } else { 
        ...
        for (child = 0; child < child_count; child++) {
          if(tree->geom->subtree_sizes[child] == 1) {
            /* Destination is a Leaf */ 
            /* Use NB Put + Sync */
            gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, children[child]), args->dst, args->dst, args->nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
          } else { 
            /* Destiantion is an Internal Node*/
            /* Use Async Long */
            gasnete_coll_p2p_signalling_putAsync(op, GASNETE_COLL_REL2ACT(op->team, children[child]), args->dst, 
...
static int gasnete_coll_pf_bcast_ScatterAllgather(gasnete_coll_op_t *op GASNETE_THREAD_FARG) {
  ...
  case 2:	/* Initiate all gather*/
      ...
      if (!gasnete_coll_generic_coll_sync((gex_Event_t*) data->private_data, 1 GASNETE_THREAD_PASS)) {
        break;
      } else {
        gex_Event_t *handle;
        uint8_t *tempspace;
  ...
  case 3:
      ...
      if (!gasnete_coll_generic_coll_sync((gex_Event_t*) data->private_data, 2 GASNETE_THREAD_PASS)) {
        break;
      }
      data->state = 4; GASNETI_FALLTHROUGH
  ...
  switch (data->state) {
    case 0:	/* Optional IN barrier */
      ...
      if (op->team->myrank == args->srcnode) {
        ...
        data->handle = gasnete_get_nb(gasneti_THUNK_TM, GASNETE_COLL_MY_1ST_IMAGE(op->team,args->dstlist, 0),
        ...
        gasnete_begin_nbi_accessregion(0,1 GASNETE_THREAD_PASS);
        {
          ...
          if (op->team->myrank < op->team->total_ranks - 1) {
            ...
            for (i = op->team->myrank + 1; i < op->team->total_ranks; ++i) {
              ...
              for (j = 0; j < limit; ++j) {
                gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team,i), *p, src, nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
                ++p;
              }
            }
          }
      ...
      if (op->team->myrank == args->srcnode) {
        ...
        for (child = 0; child < child_count; child++) {
          if(tree->geom->subtree_sizes[child] == 1 && op->team->all_images[children[child]] == 1) {
            /* Destination is a Leaf */ 
            /* Use NB Put + Sync */
            gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, children[child]), GASNETE_COLL_1ST_IMAGE(op->team,args->dstlist, children[child]), args->src, args->nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
          } else {
            /* Destiantion is an Internal Node*/
            /* Use Async Long */
        ...
        for (child = 0; child < child_count; child++) {
          if(tree->geom->subtree_sizes[child] == 1 && op->team->all_images[children[child]] == 1) {
            /* Destination is a Leaf */ 
            /* Use NB Put + Sync */
            gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, children[child]), GASNETE_COLL_1ST_IMAGE(op->team,args->dstlist, children[child]), 
      ...
      if (op->team->myrank != args->srcnode) {
        ...
        gasnete_begin_nbi_accessregion(0,1 GASNETE_THREAD_PASS);
        {
          ...
          for (i = op->team->myrank + 1; i < op->team->total_ranks; ++i, p += nbytes) {
            gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, i), dst, (void *)p, nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
          }
          /* Put to nodes to the "left" of ourself */
          ...
          for(i=0,p=0; i<child_count; i++) {
            ...
            if(op->flags & GASNET_COLL_OUT_MYSYNC) {
              ...
              if(tree->geom->subtree_sizes[i] == 1  && direct_put_ok) {
                ...
                /* Perform NB Put */
                gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, children[i]), args->dst,
                                send_arr,
        ...
        for(i=0; i<child_count; i++) {
          ...
          if(tree->geom->subtree_sizes[i]==1 && direct_put_ok) {
            ...
            gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, children[i]), args->dst, 
                            ...
                            args->nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
          } else {
        ...
        if(args->dist!=args->nbytes) {
          ...
          for(i=0; i<child_count; i++) {
            ...
            if(children[i]+tree->geom->subtree_sizes[i] <= op->team->total_ranks) {
              ...
              if(tree->geom->subtree_sizes[i] == 1 && direct_put_ok) {
                gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, children[i]), args->dst, send_arr, args->nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
              } else {
                gasnete_coll_p2p_signalling_put(op, GASNETE_COLL_REL2ACT(op->team, children[i]), 
                                                (int8_t*)op->team->scratch_segs[child].addr+op->scratchpos[i], 
                                                send_arr,
      ...
      if (op->team->myrank != args->dstnode) {
        ...
        gasnete_begin_nbi_accessregion(0,1 GASNETE_THREAD_PASS);
        {
          ...
          for (i = op->team->myrank + 1; i < op->team->total_ranks; ++i, p += args->nbytes) {
            gasnete_get_nbi(gasneti_THUNK_TM, (void *)p, GASNETE_COLL_REL2ACT(op->team,i), args->src, args->nbytes, 0 GASNETE_THREAD_PASS);
          }
      ...
      if(op->team->myrank == args->dstnode) {        
        ...
        if(parent == args->dstnode && direct_put_ok) {
          ...
          if(op->flags & GASNET_COLL_OUT_ALLSYNC) {
            ...
            data->handle = gasnete_put_nb(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, parent), gasnete_coll_scale_ptr(args->dst,(tree->geom->sibling_offset+1),args->nbytes),
      ...
      if(parent == args->dstnode && direct_put_ok) {
        ...
        */
        ...
        if(op->flags & GASNET_COLL_OUT_ALLSYNC) {
          ...
          /*parent is not waiting for the collective to finish so go ahead and just do a nonblocking put*/
          data->handle2 = gasnete_put_nb(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team, parent), gasnete_coll_scale_ptr(args->dst,(2+tree->geom->sibling_offset),args->nbytes),
                                             src_addr,
                                              args->nbytes*(tree->geom->mysubtree_size-1),
                                              GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
          gasnete_coll_save_event(&data->handle2);
        }
      } else {
    ...
    if_pt(op->team->total_ranks > 1) {
      gasnete_begin_nbi_accessregion(0,1 GASNETE_THREAD_PASS);
      {
        for(dst=op->team->myrank+1; dst<op->team->total_ranks; dst++) {
          ...
          gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team,dst), (int8_t*) args->dst + op->team->myrank*args->nbytes, args->src, args->nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
        }
    ...
    if_pt(op->team->total_ranks > 1) {
      gasnete_begin_nbi_accessregion(0,1 GASNETE_THREAD_PASS);
      {
        for(srcnode=op->team->myrank+1; srcnode<op->team->total_ranks; srcnode++) {
          ...
          gasnete_get_nbi(gasneti_THUNK_TM, gasnete_coll_scale_ptr(args->dst, srcnode, args->nbytes),
...
static int gasnete_coll_pf_gallM_DissemNoScratchSeg(gasnete_coll_op_t *op GASNETE_THREAD_FARG) {
  ...
  if(data->state >= 2 && data->state <= (dissem->dissemination_phases-1)*3+1 && op->team->total_ranks!=1) {
    ...
    if(((data->state-2) % 3) == 0) {
      /* send in this phase */
      gasneti_sync_reads();
      data->handle = gasnete_put_nb(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team,dstnode), 
                                         (int8_t*)GASNETE_COLL_1ST_IMAGE(op->team, args->dstlist, dstnode)+curr_len,
                                         (int8_t*)GASNETE_COLL_MY_1ST_IMAGE(op->team,args->dstlist, op->flags),
                                         curr_len, GEX_EVENT_DEFER, 0
                                         GASNETE_THREAD_PASS);
      gasnete_coll_save_event(&data->handle);
#if 0
      gasnete_coll_p2p_signalling_put(op, GASNETE_COLL_REL2ACT(op->team,dstnode), 
...
static int gasnete_coll_pf_exchg_Put(gasnete_coll_op_t *op GASNETE_THREAD_FARG) {
  ...
  case 1: /* fire off all the nonblocking puts*/
    ...
    for(i=op->team->myrank+1; i<op->team->total_ranks; i++) {
      gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team,i), ((int8_t*) args->dst) + op->team->myrank*args->nbytes, ((int8_t*) args->src)+i*args->nbytes, args->nbytes, GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
    } 
...
#define IDXP1_EXPR ((((phase+1)%2)*(dissem->dissemination_radix-1) + (j))*offset)
    ...
    if(((data->state-2) % 4) == 1) {
      ...
      for(j=0; j<h; j++) {
        ...
        gasnete_coll_pack_all_to_all_msg(scratch2, (int8_t*)scratch1+IDX_EXPR,args->nbytes*op->team->my_images*op->team->my_images,
                                         phase, dissem->dissemination_radix, j+1, op->team->total_ranks);
        gasnete_put_nbi(gasneti_THUNK_TM, GASNETE_COLL_REL2ACT(op->team,destnode),
                        (int8_t*)op->team->scratch_segs[destnode].addr+op->scratchpos[0]+IDXP1_EXPR,
                        (int8_t*)scratch1+IDX_EXPR,
                        nblocks*args->nbytes*op->team->my_images*op->team->my_images,
                        GEX_EVENT_DEFER, 0 GASNETE_THREAD_PASS);
        /*gasnete_coll_p2p_counting_put(op, GASNETE_COLL_REL2ACT(op->team,destnode), */
        /*(int8_t*)op->team->scratch_segs[destnode].addr+op->scratchpos[0]+IDXP1_EXPR, (int8_t*)scratch1+IDX_EXPR,*/
        /*                                      nblocks*args->nbytes*op->team->my_images*op->team->my_images, phase*2+1);*/
        ...
        for(i=0; i<child_count; i++, src_addr+=nbytes) {
          ...
          switch(s) {
            ...
            case 1: {
              ...
              ((gex_Event_t*) data->private_data)[i] = gasnete_get_nb(gasneti_THUNK_TM, (void*) src_addr, GASNETE_COLL_REL2ACT(op->team, children[i]), 
                                                                               child_scratch, args->nbytes, 0 GASNETE_THREAD_PASS);
              gasnete_coll_save_event(&((gex_Event_t*) data->private_data)[i]);
              
            }
            state[i]++; GASNETI_FALLTHROUGH
...
static int gasnete_coll_pf_reduce_TreePutSeg(gasnete_coll_op_t *op GASNETE_THREAD_FARG) {
  ...
  switch (data->state) {
    ...
    case 1:	/* Initiate data movement */
    {
      ...
      handle_vec->handles = gasneti_malloc(sizeof(gex_Event_t)*num_elem_segs);
