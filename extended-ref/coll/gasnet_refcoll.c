...
#if GASNET_PAR
  if_pf (flags & GASNET_COLL_LOCAL && team->multi_images_any &&
         !(flags & (GASNETE_COLL_SUBORDINATE|GASNET_COLL_NO_IMAGES))) {
    gasneti_fatalerror("GASNET_COLL_LOCAL is no longer supported with multiple images");
  }
#endif
