...
void gasnete_coll_set_dissemination_order(gasnete_coll_local_tree_geom_t *geom, int gasnete_coll_tree_mynode, int gasnete_coll_tree_nodes) {
  ...
  proc_list = (gasnetex_rank_t*)gasneti_malloc(sizeof(gasnetex_rank_t)*lognp);
...
static gasnetex_rank_t multarr(int *arr, int nelem){
  int ret=1; int i;
  for(i=0; i<nelem; i++) {
    ret*=arr[i];
  }
...
static tree_node_t make_recursive_tree(tree_node_t *nodes, gasnetex_rank_t num_nodes, int radix) {
  gasnetex_rank_t i,j;
  int num_children=0;
...
static tree_node_t make_nary_tree(tree_node_t *nodes, gasnetex_rank_t num_nodes, int radix) {
  gasnetex_rank_t num_children=0;
  int i,j;
...
static tree_node_t find_node(tree_node_t tree, gasnetex_rank_t id) {
  gasnetex_rank_t i;
...
gasnete_coll_local_tree_geom_t *gasnete_coll_tree_geom_create_local(gasnete_coll_tree_type_t in_type, int rootrank, gasnete_coll_team_t team, gasnete_coll_tree_geom_t *base_geom)  {
  ...
  if(rootrank != team->myrank) {
    ...
    /***** THIS NEEDS TO BE TAKEN OUT
      ...
      ****/
    geom->dfs_order = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*team->total_ranks);
    for(i=0; i<team->total_ranks; i++) {
      geom->dfs_order[i] = (i+rootrank)%team->total_ranks;
    }
  }
  geom->seq_dfs_order = 1;
  geom->child_list = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*geom->child_count);
  geom->subtree_sizes = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*geom->child_count);
  geom->child_offset = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*geom->child_count);
  geom->grand_children = (gasnetex_rank_t*)gasneti_malloc(sizeof(gasnetex_rank_t)*geom->child_count);
...
gasnete_coll_local_tree_geom_t *gasnete_coll_local_tree_geom_fetch(gasnete_coll_tree_type_t type, gasnetex_rank_t root,  gasnete_coll_team_t team) {
  ...
  if(team->myrank != ret->root) {
    ...
    gasnetex_rank_t *tmp = gasneti_calloc(1+count, sizeof(gasnetex_rank_t));
    memcpy(tmp, GASNETE_COLL_TREE_GEOM_CHILDREN(ret), count*sizeof(gasnetex_rank_t));
...
gasnete_coll_dissem_info_t *gasnete_coll_build_dissemination(int r, gasnete_coll_team_t team) {
  ...
  for(i=0; i<w; i++) {
    ...
    ret->exchange_order[i].elem_list = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*(h-1));
    for(j=1; j<h; j++) {
...
/* tree building code*/
static int gasnete_coll_build_tree_mypow(gasnetex_rank_t base, int pow) {
  int ret = 1;
  while(pow!=0) {
    ret *=base;
    pow--;
...
static int gasnete_coll_build_tree_mylog2(gasnetex_rank_t num) {
  unsigned int ret=0;
  while (num >= 1) {
    ret++;
    num = num >> 1;
...
static int gasnete_coll_build_tree_mylogn(gasnetex_rank_t num, int base) {
  int ret=1;
  int mult = base;
  while (num > mult) {
    ret++;
...
gasnete_coll_dissem_info_t *gasnete_coll_build_dissemination(int r, gasnete_coll_team_t team) {
  ...
  ret->ptr_vec = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*(w+1));
  ret->ptr_vec[0] = 0;
  ...
  ret->exchange_out_order = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*(ret->ptr_vec[w]));
  ret->exchange_in_order = (gasnetex_rank_t*) gasneti_malloc(sizeof(gasnetex_rank_t)*(ret->ptr_vec[w]));
