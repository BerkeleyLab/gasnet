...
*/
extern void gasnete_coll_p2p_long_reqh(gex_AM_Token_t token, void *buf, size_t nbytes,
...
*/
extern void gasnete_coll_p2p_med_reqh(gex_AM_Token_t token, void *buf, size_t nbytes,
                                      gex_AM_Arg_t team_id,
                                      gex_AM_Arg_t sequence,
                                      gex_AM_Arg_t count,
                                      gex_AM_Arg_t offset,
...
extern void gasnete_coll_p2p_med_counting_reqh(gex_AM_Token_t token, void *buf, size_t nbytes,
                                               gex_AM_Arg_t team_id,
                                               gex_AM_Arg_t sequence,
                                               gex_AM_Arg_t offset,
                                               gex_AM_Arg_t idx,
...
*/
extern void gasnete_coll_p2p_med_tree_reqh(gex_AM_Token_t token, void *buf, size_t nbytes,
                                           gex_AM_Arg_t team_id,
                                           gex_AM_Arg_t sequence) {
  gasnete_coll_p2p_t *p2p = gasnete_coll_p2p_get(team_id, sequence);
...
/* No payload to deliver, just updates 1 or more states
   ...
   state: value to assign to states [offset, offset+count)
*/
extern void gasnete_coll_p2p_short_reqh(gex_AM_Token_t token,
                                        gex_AM_Arg_t team_id,
                                        gex_AM_Arg_t sequence,
                                        gex_AM_Arg_t count,
                                        gex_AM_Arg_t offset,
...
extern void gasnete_coll_p2p_advance_reqh(gex_AM_Token_t token,
                                          gex_AM_Arg_t team_id,
                                          gex_AM_Arg_t sequence,
                                          gex_AM_Arg_t idx) {
...
extern void gasnete_coll_p2p_put_and_advance_reqh(gex_AM_Token_t token, void *buf, size_t nbytes,
                                                  gex_AM_Arg_t team_id,
                                                  gex_AM_Arg_t sequence,
                                                  gex_AM_Arg_t idx) {
...
extern void gasnete_coll_p2p_seg_put_reqh(gex_AM_Token_t token, void *buf, size_t nbytes,
                                          gex_AM_Arg_t team_id,
                                          gex_AM_Arg_t sequence,
                                          gex_AM_Arg_t seg_id) {
...
GASNETI_INLINE(gasnete_coll_p2p_memcpy_reqh_inner)
     void gasnete_coll_p2p_memcpy_reqh_inner(gex_AM_Token_t token, void *buf, size_t nbytes,
                                             void *dest,
                                             gex_AM_Arg_t team_id,
                                             gex_AM_Arg_t sequence,
...
void gasnete_coll_p2p_signalling_put(gasnete_coll_op_t *op, gasnetex_rank_t dstnode, void *dst,
  ...
  gex_AM_RequestLong(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_long_reqh),
                         src, nbytes, dst, GASNETEX_EVENT_NOW, 0, team_id, op->sequence, 1, offset, state);
}
...
void gasnete_coll_p2p_counting_put(gasnete_coll_op_t *op, gasnetex_rank_t dstnode, void *dst,
  ...
  gex_AM_RequestLong(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_put_and_advance_reqh),
                         src, nbytes, dst, GASNETEX_EVENT_NOW, 0, team_id, seq_num, idx);
}
/* Put up to gex_AM_LUBRequestLong() bytes, signalling the recipient */
/* Returns immediately even if the local buffer is not yet reusable */
...
void gasnete_coll_p2p_sig_seg_put(gasnete_coll_op_t *op, gasnetex_rank_t dstnode, void *dst,
  ...
  gex_AM_RequestLong(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_seg_put_reqh),
                         src, nbytes, dst, GASNETEX_EVENT_NOW, 0, team_id, seq_num, seg_id);
}
...
void gasnete_coll_p2p_eager_putM(gasnete_coll_op_t *op, gasnetex_rank_t dstnode,
  ...
  limit = gex_AM_LUBRequestMedium() / size;
  if_pf (count > limit) {
    size_t nbytes = limit * size;

    do {
      gex_AM_RequestMedium(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_med_reqh),
                               src, nbytes, GASNETEX_EVENT_NOW, 0, team_id, op->sequence, limit, offset, state, size);
      offset += limit;
  ...
  gex_AM_RequestMedium(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_med_reqh),
                           src, count * size, GASNETEX_EVENT_NOW, 0, team_id, op->sequence, count, offset, state, size);
}
...
void gasnete_coll_p2p_eager_put_tree(gasnete_coll_op_t *op, gasnetex_rank_t dstnode,
  ...
  gex_AM_RequestMedium(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_med_tree_reqh),
                           src, size, GASNETEX_EVENT_NOW, 0, team_id, seq_num);
...
void gasnete_coll_p2p_change_states(gasnete_coll_op_t *op, gasnetex_rank_t dstnode,
  ...
  gex_AM_RequestShort(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_short_reqh), 0,
...
void gasnete_coll_p2p_memcpy(gasnete_coll_op_t *op, gasnetex_rank_t dstnode, void *dst,
  ...
  gex_AM_RequestMedium(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_memcpy_reqh),
                           src, nbytes, GASNETEX_EVENT_NOW, 0, PACK(dst), team_id, op->sequence, 1);
}
...
extern void gasnete_coll_p2p_counting_eager_put(gasnete_coll_op_t *op, gasnetex_rank_t dstnode,
  ...
  gex_AM_RequestMedium(NULL, dstnode, gasneti_handleridx(gasnete_coll_p2p_med_counting_reqh),
                           src, nbytes, GASNETEX_EVENT_NOW, 0, team_id, op->sequence, offset, idx, offset_size);
}
                                    ...
                                    GASNETE_THREAD_FARG)
{
  ...
  if (team->my_images*nbytes <=  gasnete_coll_get_dissem_limit(team->autotune_info, GASNET_COLL_GATHER_ALLM_OP, flags) &&
    ...
    if((flags & GASNET_COLL_DST_IN_SEGMENT) && (flags & GASNET_COLL_SINGLE) && team->fixed_image_count && team->my_images*nbytes <= gex_AM_LUBRequestLong()) {
