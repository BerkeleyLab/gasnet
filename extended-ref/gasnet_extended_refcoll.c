  ...
  if_pt (op != NULL) {
    ...
    if (op->flags & (GASNET_COLL_IN_ALLSYNC | GASNET_COLL_IN_MYSYNC)) {
      ...
      if((int) gasneti_atomic_read(&GASNETE_COLL_GENERIC_DATA(op)->threads.remaining, 0) <=0) {
        fprintf(stderr, "threads remaining before dec: %d (has to be > 0!!!) seq: %d\n", (int) gasneti_atomic_read(&GASNETE_COLL_GENERIC_DATA(op)->threads.remaining, 0), op->sequence);
                                ...
                                GASNETE_THREAD_FARG)
{
  ...
  return (*((gasnete_coll_scatter_fn_ptr_t) (impl->fn_ptr)))(team, dst, srcimage, src, nbytes, nbytes, flags, impl, sequence GASNETE_THREAD_PASS);
}
#endif
...
gasnete_coll_scatterM_nb_default(gasnet_team_handle_t team,
  ...
  return (*((gasnete_coll_scatterM_fn_ptr_t) (impl->fn_ptr)))(team, dstlist, srcimage, src, nbytes, nbytes, flags, impl, sequence GASNETE_THREAD_PASS);
}
#endif
                        ...
                        GASNETE_THREAD_FARG)
{
  ...
  return gasnete_coll_generic_exchange_nb(team, dst, src, nbytes, flags,
                                          ...
                                          NULL, NULL, team->total_images GASNETE_THREAD_PASS);
}
                                 ...
                                 GASNETE_THREAD_FARG)
{
  ...
  if (nbytes <=  gasnete_coll_get_dissem_limit(team->autotune_info, GASNET_COLL_EXCHANGE_OP, flags) &&
    ...
    if(flags & GASNET_COLL_SINGLE) {
      return gasnete_coll_exchg_Put(team, dst, src, nbytes, flags, sequence GASNETE_THREAD_PASS);
    } else{
                               ...
                               int flags, uint32_t sequence GASNETE_THREAD_FARG)
{
  ...
  flags = gasnete_coll_segment_check(team, flags, 1, dstimage, dst, nbytes,
                                     0, 0, src, nbytes);
