...
/* gasnet_image_t must be large enough to index all threads that
 * participate in collectives.	A conduit may override this if a
 * smaller type will suffice.	 However, types larger than 32-bits
 * won't pass as AM handler args.	 So, for a larger type, many default
 * things will require overrides.
 */

#define GASNET_IMAGE_UNDEFINED UINT_MAX /* gasnet_image_t is uint32_t, UINT_MAX is defined in limits.h */
...
extern int gasnete_coll_smp_tune_barriers;
...
extern void gasnete_coll_active_ins(gasnete_coll_team_t team, gasnete_coll_op_t *op);
extern void gasnete_coll_active_del(gasnete_coll_team_t team, gasnete_coll_op_t *op);
/*---------------------------------------------------------------------------------*/
...
/* Map the relative rank of the process in the team to its gasnet node
   id.  This should not be confused with the image id mapping, which
   is for threads. */
#define GASNETE_COLL_REL2ACT(TEAM, RANK) \
  ((TEAM) == GASNET_TEAM_ALL ? (RANK) : (TEAM)->rel2act_map[RANK])
...
/* Thread-specific data for each team */
typedef struct gasnete_coll_team_threaddata_t_ {
...
typedef struct gasnete_coll_team_mailbox_t_ {
  volatile int empty;
  void *data;
} gasnete_coll_team_mailbox_t;

/* Type for collective teams: */
struct gasnete_coll_team_t_ {
	uint32_t team_id; /**< a unique team id */
	int global_team; /**< 1 if the team include all images in GASNET_TEAM_ALL */
  uint32_t sequence; /**< arbitrary non-zero starting value */
  
	gasnet_node_t total_ranks; /**< total number of processes in the team */
  gasnet_node_t myrank; /**< my relative node rank in this team */
	gasnet_node_t *rel2act_map; /**< map the process ranks in the team to the process ranks in GASNET_TEAM_ALL */
  ...
  gasneti_mutex_t gasnete_coll_active_lock; //  = GASNETI_MUTEX_INITIALIZER;
	...
	gasnet_image_t max_images; /**< the max number of images of any process */
	gasnet_image_t *all_images; /**< array of count of images in each process, whose length is total_ranks */
	gasnet_image_t *all_offset; /**< array of the offsets of the first images of each process, whose length is total_ranks */
	gasnet_image_t my_images;	/**< count of local images, which equals to all_images[myrank] */
	gasnet_image_t my_offset;	/**< count of images before my first image, which equals to all_offset[myrank] */
  ...
  volatile uint32_t threads_sequence;	/* the thread sequence is for figuring out the first thread arrival, which isindependent of collective op sequence space */
  gasneti_mutex_t gasnete_coll_threads_mutex;
  ...
  /* scratch segments allocated on team creation */
	...
	gasnete_coll_scratch_status_t* scratch_status; /**< scratch space management */
		
	gasnete_coll_autotune_info_t* autotune_info; /**< autotuning info */
...
/* Treat the eager buffer space at dstnode as an array of elements of
 * length 'size'.	 Copy 'count' elements to that buffer, starting at
 * element 'offset' at the destination.	 Set the corresponding entries
 * of the state array to 'state'.
 */
 ...
 * Copy 'count' elements to that buffer, starting at element 'offset'
 * at the destination.	Set the corresponding entries of the state
 * array to 'state'.
 */
...
/* Treat the eager buffer space on each node as an array of elements
 * of length 'size'.	Send (to all but the local node) one element to
 * position 'offset' of that array.	 Set the corresponding entries of
 * the state array to 'state'.	When 'scatter' == 0, the same local
 * element is sent to all nodes (broadcast).	When 'scatter' != 0, the
 * source is an array with elements of length 'size', with the ith
 * element sent to node i.
 */
...
GASNETI_INLINE(gasnete_coll_local_broadcast2)
void gasnete_coll_local_broadcast2(size_t count, uint32_t const dstindex[], void * const dstlist[], const void *src, size_t nbytes) 
{
	size_t i;
	for (i=0; i<count; i++) 
		GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(dstlist[dstindex[i]], src, nbytes);
...
GASNETI_INLINE(gasnete_coll_local_scatter2)
void gasnete_coll_local_scatter2(size_t count, uint32_t  const dstindex[], void * const dstlist[], const void *src, size_t nbytes) 
{
	const uint8_t *src_addr = (const uint8_t *)src;
  size_t i;
  
	for (i=0; i<count; i++,	src_addr += nbytes)
		GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(dstlist[dstindex[i]], src_addr, nbytes);
...
GASNETI_INLINE(gasnete_coll_local_gather2)
void gasnete_coll_local_gather2(size_t count, void * dst, uint32_t const srcindex[], void * const srclist[], size_t nbytes) 
{
	uint8_t *dst_addr = (uint8_t *)dst;
  size_t i;

	gasneti_sync_reads();
	for (i=0; i<count; i++, dst_addr += nbytes)
		GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(dst_addr, srclist[srcindex[i]], nbytes);
...
GASNETI_INLINE(gasnete_coll_local_reduce2)
void gasnete_coll_local_reduce2(size_t count, void * dst, uint32_t const srcindex[], 
                                void * const srclist[], size_t elem_size, 
                                size_t elem_count, gasnet_coll_fn_handle_t func, 
                                int func_arg) 
{
	gasnet_coll_reduce_fn_t reduce_fn = gasnete_coll_fn_tbl[func].fnptr;
	uint32_t red_fn_flags = gasnete_coll_fn_tbl[func].flags;
	...
	GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(dst, srclist[0], nbytes);
	for(i=1; i<count; i++) {
		(*reduce_fn)(dst, elem_count, dst, elem_count, srclist[srcindex[i]], elem_size, red_fn_flags, reduce_args);
	}
	gasneti_sync_writes();	/* Ensure result is visible on all threads */
}
...
typedef struct {
	gasnet_image_t my_image; /**< my imageid in GASNET_TEAM_ALL */
	gasnet_image_t my_local_image; /**< my local imageid (= my_image-my_offset) in GASNET_TEAM_ALL */
...
/**
 * Look up the thread specific data for the team 
 */
gasnete_coll_team_threaddata_t *
...
/* The flags GASNET_COLL_SRC_IN_SEGMENT and GASNET_COLL_DST_IN_SEGMENT
 * are just assertions from the caller.	 If they are NOT set, we will
 * try to determine (when possible) if the addresses are in-segment to
 * allow a one-sided implementation to be used.
 * gasnete_coll_segment_check and gasnete_coll_segment_checkM return a
 * new set of flags.
 */
...
int _gasnete_coll_segment_check_aux(gasnete_coll_team_t team, int rooted, gasnet_image_t root, const void *addr, size_t len) {
	...
	if (rooted) {
		...
		return gasnete_coll_in_segment(gasnete_coll_image2rank(team, root), addr, len);
	} else {
...
#if GASNET_PAR
extern void gasnete_coll_threads_lock(gasnet_team_handle_t team, int flags GASNETE_THREAD_FARG);
extern void gasnete_coll_threads_unlock(gasnet_team_handle_t team GASNETE_THREAD_FARG);
int gasnete_coll_threads_first(gasnet_team_handle_t team GASNETE_THREAD_FARG);
extern gasnet_coll_handle_t gasnete_coll_threads_get_handle(gasnet_team_handle_t team GASNETE_THREAD_FARG);
