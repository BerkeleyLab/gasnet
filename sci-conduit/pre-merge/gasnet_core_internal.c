...
gasnet_node_t gasnetc_mynode = 0;
gasnet_node_t gasnetc_nodes = 2;

/********************************************************
					Global Variables
********************************************************/
// sci_error_t						gasnetc_sci_error;									//SCI status report of success and failure
...
void gasnetc_sci_internal_Barrier()
{
	...
	*/
	...
	SCICreateSegment(barrier_sd, &barrier_local, seg_ID, gasnetc_nodes, /*create as many bytes as there are nodes*/ 
		...
		if (gasnetc_sci_error != SCI_ERR_OK) 
		{
			printf("Barrier, SCICreateSegment failed - Error code: 0x%x\n",gasnetc_sci_error);
			fflush(NULL);
		}
...
int gasnetc_parseSCIIds(FILE *node_info, const int number)
{
	...
	for(index = 0; index < number; index++)
	{
		...
		{
			/*printf("Node: %d, char: %c.", scanned, ch);
			fflush(NULL);*/
	...
	for(index = 0; index < number; index++)
	{
		printf("%d ", gasnetc_sci_SCI_Ids[index]);
	}
	printf("\n");
	*/
	return 1;
...
int gasnetc_get_free_mem()
{
	...
	#if defined(GASNET_SEGMENT_FAST)
	{
		...
		return GASNETC_SCI_FAST_SEG; /*all will have this value, so no need to write to file*/
	}
	#endif
	...
	mod = size%GASNET_PAGESIZE; // make it an even multiple of GASNET_PAGESIZE (should be 4 for SCI)
	size = size - mod;
	...
	printf("Local free mem is %d", size);
	return size;
}
...
int gasnetc_getSCIglobal_seg(int number)
{
	...
	#endif
	/*
	printf("My segment size is: %d\n", gasnetc_sci_max_local_seg);
	fflush(NULL);*/
	...
	Table_Sizes = (unsigned int *) malloc( sizeof(unsigned int)* gasnetc_nodes);
	//get info, place into the table
	/*	printf("Table of segment sizes looks like:\n");
	fflush(NULL);*/
	for(index = 0; index < gasnetc_nodes; index++)
	{
		segment_size = (unsigned int*) gasnetc_sci_local_mem[index];
		Table_Sizes[index] = segment_size[offset];
		//printf("%d, %d\n", index,Table_Sizes[index] );
	}
	...
	/*	printf("MIN is %d. Global size is : %d.\n", min,gasnetc_sci_max_global_seg );*/
	return min;
}
...
gasnet_node_t gasnetc_SCI_Create_Connections(int number)
{
	...
	gasnetc_sci_local_mem = (void **)malloc( (number +2) * sizeof(void*) );
	...
	for (index = 0; index < (number +2); index++)
	{
		SCIClose(gasnetc_sci_sd[index], GASNETC_SCI_NO_FLAGS, &gasnetc_sci_error);
	}*/
...
int gasnetc_SCI_connect_cmd(int number)
{
	...
	gasnetc_sci_sd_remote = (sci_desc_t*)malloc(number* sizeof(sci_desc_t));
	for (index = 0; index < number ; index++)
	{
	...
	gasnetc_sci_remote_mem = (void**)malloc( (number +2) * sizeof(void*) );
	...
	gasnetc_sci_gb_sd = (sci_desc_t*)malloc(sizeof(sci_desc_t) * (number));
	for (index = 0; index < number ; index++)
	{
	...
	gasnetc_sci_remoteSegment_gb = (sci_remote_segment_t*)malloc(sizeof(sci_remote_segment_t)*number);
	for (index = 0; index < number ; index++)
	{
	...
	gasnetc_sci_global_ready = (void*)malloc( number * sizeof(void*) );
	gasnetc_sci_remoteMap_gb = (sci_map_t*)malloc( number * sizeof(sci_map_t) );
...
unsigned int gasnetc_SCIInit(gasnet_node_t * my_node)
{
	...
	if(node_info == NULL)
	{
		//printf("Could not get access to initalization file.\n");
	...
	gasnetc_sci_SCI_Ids = (unsigned int *) malloc( sizeof(unsigned int) * number );
	...
	gasnetc_sci_sd = malloc( sizeof(sci_desc_t) * (number + 2) );
...
void* gasnetc_create_gasnetc_sci_seg(uintptr_t *segsize, int index)
{
	...
	gasnetc_sci_remoteSegment_long = (sci_remote_segment_t *) malloc(sizeof(sci_remote_segment_t) * number);
	gasnetc_sci_sd_long = (sci_desc_t *) malloc(sizeof(sci_desc_t) * number);
...
void gasnetc_get_SegInfo( gasnet_seginfo_t * SEG_INFO, uintptr_t segsize,  void * segbase)
{
	...
	TEMP = (gasnet_seginfo_t *) malloc(sizeof(gasnet_seginfo_t));//allocate enough space for one
	...
	for(index = 0; index < gasnetc_nodes; index++)
	{
		...
		//printf("%d, %d, %u\n", index, SEG_INFO[index].size, SEG_INFO[index].addr);
	}
	//fflush(NULL);
}
...
{
	int i;
	// msg_loc_status = (uint8_t *) gasneti_malloc ((sizeof (uint8_t)) * (GASNETC_SCI_MAX_REQUEST_MSG * gasnetc_nodes * 2));
	gasnetc_sci_msg_loc_status = (uint8_t *) malloc ((sizeof (uint8_t)) * (GASNETC_SCI_MAX_REQUEST_MSG * gasnetc_nodes * 2));
...
void print_work_queue_info ()
{
	if (gasnetc_sci_first == NULL)
	{
		printf ("WORK QUEUE IS CURRENTLY EMPTY\n");
	}
	else
	{
		gasnetc_command_receiver_t *msg_ptr = (gasnetc_command_receiver_t *) gasnetc_sci_first;
		do
		{	
			...
			if ((msg_AM_type == GASNETC_SCI_SHORT) || (msg_AM_type == GASNETC_SCI_MEDIUM))
			{
				...
				printf ("     Source ID = %d, Msg_Number = %d\n, Next = %p\n", SM_msg->token.source_id, SM_msg->token.msg_number, SM_msg->next);
				msg_ptr = (gasnetc_command_receiver_t *) SM_msg->next;
			}
			else if (msg_AM_type == GASNETC_SCI_LONG)
			{
				gasnetc_Long_command_receiver_t *L_msg = (gasnetc_Long_command_receiver_t *) msg_ptr;
				printf ("     Source ID = %d, Msg_Number = %d\n, Next = %p\n", L_msg->token.source_id, L_msg->token.msg_number, L_msg->next);
				msg_ptr = (gasnetc_command_receiver_t *) L_msg->next;
			} 
			else if (msg_AM_type == GASNETC_SCI_CONTROL)
			{
				printf ("     Source ID = %d, Msg_Number = %d\n, Next = %p\n", msg_ptr->token.source_id, msg_ptr->token.msg_number, msg_ptr->next);
				msg_ptr = (gasnetc_command_receiver_t *) msg_ptr->next;
			}
		} while (msg_ptr != NULL);
	}
}
...
void gasnetc_print_command (gasnetc_command_t *command)
{
	printf ("Handler # = %d \n", gasnetc_get_msg_handler(command->header));
	if (gasnetc_get_msg_type (command->header) == 0)
	{ printf ("Msg type = REQUEST \n");}
	else 
	{ printf ("Msg type = REPLY \n");}
	if (gasnetc_get_AM_type (command->header) == 0)
	{ printf ("AM type = SHORT \n");}
	else if (gasnetc_get_AM_type (command->header) == 1)
	{ printf ("AM type = MEDIUM \n");}
	else if (gasnetc_get_AM_type (command->header) == 2)
	{ printf ("AM type = LONG \n");}
	else if (gasnetc_get_AM_type (command->header) == 3)
	{ printf ("AM type = CONTROL \n");}
	printf ("Num Arg = %d \n", gasnetc_get_msg_num_arg (command->header));
}

void gasnetc_print_sm_command (gasnetc_ShortMedium_command_t *smcommand, int *Pay)
{
	gasnetc_print_command (&(smcommand->header));
	if (gasnetc_get_AM_type (smcommand->header.header) == 1)
	{ printf ("Payload Size = %d \n", smcommand->payload_size);}
	if (gasnetc_get_msg_num_arg (smcommand->header.header) > 0)
	{
		int i;
		for (i = 0; i < gasnetc_get_msg_num_arg (smcommand->header.header); i++)
		{
			printf ("Arg[%d] = %d, ", i, smcommand->args[i]);
		}
		printf ("\n");
	}
	if ((Pay != NULL) && (gasnetc_get_AM_type (smcommand->header.header) == 1))
	{
		printf ("PAYLOAD DATA \n");
		int Max = (smcommand->payload_size) / 4;
		int i;
		for (i = 0; i < Max; i++)
		{
			printf ("payload[%d] = %d, ", i, Pay[i]);
		}
		printf ("\n");
	}
}

void gasnetc_print_l_command (gasnetc_Long_command_t *lcommand)
{
	gasnetc_print_command (&(lcommand->header));
	if (gasnetc_get_msg_num_arg (lcommand->header.header) > 0)
	{
		int i;
		for (i = 0; i < gasnetc_get_msg_num_arg (lcommand->header.header); i++)
		{
			printf ("Arg[%d] = %d, ", i, lcommand->args[i]);
		}
		printf ("\n");
	}
	if (lcommand->payload == NULL)
	{
		lcommand->payload = gasnetc_sci_local_mem[gasnetc_nodes];
	}
	printf ("Payload ptr = %p \n",lcommand->payload);
	printf ("Payload Size = %d \n",lcommand->payload_size);
	printf ("PAYLOAD DATA \n");
	int Max = lcommand->payload_size;
	int i;
	uint8_t *Pay = lcommand->payload;
	for (i = 0; i < Max; i++)
	{
		printf ("payload[%d] = %d, ", i, Pay[i]);
	}
											...
											uint8_t msg_type, uint8_t AM_type, size_t size, uint8_t num_args, gasnet_handlerarg_t args[])
{
	...
	//gasnetc_print_sm_command (temp, args);
}
						...
						bool *remote_msg_flag_addr, void *long_payload)
{
	...
	if ((AM_type == GASNETC_SCI_SHORT) || (AM_type == GASNETC_SCI_MEDIUM))
	{
		...
		command = malloc (sizeof(gasnetc_ShortMedium_command_t));
	...
	else if (AM_type == GASNETC_SCI_LONG)
	{
		...
		command = malloc (sizeof(gasnetc_Long_command_t));
	...
	else
	{
		...
		command = malloc (sizeof(gasnetc_command_t));
...
{	 
	...
	gasnetc_sci_local_dma_map = (sci_map_t *) malloc ((sizeof(sci_map_t)) * GASNETC_SCI_NUM_DMA_QUEUE);
	gasnetc_sci_local_dma_segment = (sci_local_segment_t *) malloc ((sizeof(sci_local_segment_t)) * GASNETC_SCI_NUM_DMA_QUEUE);
	gasnetc_sci_local_dma_queue = (sci_dma_queue_t *) malloc ((sizeof(sci_dma_queue_t)) * GASNETC_SCI_NUM_DMA_QUEUE);
	gasnetc_sci_local_dma_sd = (sci_desc_t *) malloc ((sizeof(sci_desc_t)) * GASNETC_SCI_NUM_DMA_QUEUE);
	gasnetc_sci_local_dma_addr = malloc ((sizeof(void *)) * GASNETC_SCI_NUM_DMA_QUEUE);
...
int gasnetc_remove_dma_queues ()
{
	...
	for (i = 0; i < GASNETC_SCI_NUM_DMA_QUEUE; i++)
	{
		...
		if (error != SCI_ERR_OK) 
		{
			printf("SCIRemoveDMAQueue() failed - Error code 0x%x\n",error);
...
{ 
	gasneti_mutex_destroy(&msg_loc_mutex);
	gasneti_mutex_destroy(&wq_lock);

	// gasneti_free (msg_loc_status);
	...
	free(gasnetc_sci_sd);
	free(gasnetc_sci_gb_sd);
	free(gasnetc_sci_sd_remote);
	free(gasnetc_sci_sd_long);
	free(gasnetc_sci_localSegment);
	free(gasnetc_sci_remoteSegment);
	free(gasnetc_sci_remoteSegment_gb);
	free(gasnetc_sci_remoteSegment_long);
	free(gasnetc_sci_localMap);
	free(gasnetc_sci_remoteMap);
	free(gasnetc_sci_remoteMap_gb);
	free(gasnetc_sci_remoteNodeId);
	free(gasnetc_sci_localSegmentId);
	free(gasnetc_sci_remoteSegmentId);
	free(gasnetc_sci_remoteSegmentId_gb);
	free(gasnetc_sci_SCI_Ids);
	free(gasnetc_sci_local_mem);
	free(gasnetc_sci_remote_mem);
	free(gasnetc_sci_global_ready);	
	free(gasnetc_sci_msg_loc_status);	
	free(gasnetc_seginfo);
