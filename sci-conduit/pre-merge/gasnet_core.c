...
int GASNETC_SCI_FORCE_SCAN_THRESHOLD = 200;
...
static void gasnetc_bootstrapBarrier() {
   ...
   */
	...
	//sleep(3); //just for testing, remove after fix
}
...
static int gasnetc_init(int *argc, char ***argv) {
  ...
  #elif defined(GASNET_SEGMENT_EVERYTHING) //currently not implemented
...
*/
  if (!gasneti_init_done) 
	  gasneti_fatalerror("Already initialized!\n");
    //GASNETI_RETURN_ERRR(NOT_INIT, "GASNet attach called before init");
  if (gasneti_attach_done) 
	  gasneti_fatalerror("Already Attached!\n");
  ...
  /*  register handlers */
  { /*  core API handlers */
  /* NOT quite sure where gasnetc_get_handlertable() is
    ...
    if (gasnetc_reghandlers(ctable, len, 1, 63, 0, &numreg) != GASNET_OK)
      //GASNETI_RETURN_ERRR(RESOURCE,"Error registering core API handlers");
		gasneti_fatalerror("Bad handler registries.\n");
    //assert(numreg == len);
	*/
  }
  ...
  //UNCOMMENT FOR INTEGRATION gasneti_registerSignalHandlers(gasneti_defaultSignalHandler);
  ...
  //gasnetc_seginfo = (gasnet_seginfo_t *)gasneti_malloc(gasnetc_nodes*sizeof(gasnet_seginfo_t));
	gasnetc_seginfo = (gasnet_seginfo_t *)malloc(gasnetc_nodes*sizeof(gasnet_seginfo_t));
...
extern void gasnetc_exit(int exitcode) {
  ...
  */  
  ...
  abort(); //UNCOMMENT FOR ACTUAL INTEGRATION
}
...
extern int gasnetc_getSegmentInfo(gasnet_seginfo_t *seginfo_table, int numentries) 
{
	...
	if (numentries < gasnetc_nodes) 
		gasneti_fatalerror("less numentries than gasnet nodes.\n");
		//GASNETI_RETURN_ERR(BAD_ARG);
	...
	if (msg_addr != NULL)
	{
		...
		else 
		{
			...
			if (msg_AM_type == GASNETC_SCI_SHORT)
			{
				...
				gasnetc_print_sm_command(Short_msg, NULL);
			...
			else 
			{
				if (msg_AM_type == GASNETC_SCI_MEDIUM)
				{
					...
					gasnetc_print_sm_command(Medium_msg, msg_payload);
				...
				else if (msg_AM_type == GASNETC_SCI_LONG)
				{
					...
					gasnetc_print_l_command(Long_msg);
