                                                     ...
                                                     GASNETE_THREAD_FARG) 
{
  ...
  if(!(flags & GASNET_COLL_IN_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
  
  // \todo: td->my_local_image only works for TEAM_ALL 
  GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(dstlist[team_td->my_local_image], src, nbytes);

  if(!(flags & GASNET_COLL_OUT_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
...
{  
  ...
  smp_coll_broadcast_tree_flag(team_td->smp_coll_handle, team->my_images,
                               dstlist, src, nbytes, flags, 
                               coll_params->param_list[0]);
                              ...
                              GASNETE_THREAD_FARG) 
{
  ...
  if (team_td->my_local_image == dstimage) {
...
#endif
  if(!(flags & GASNET_COLL_IN_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
  GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(dstlist[team_td->my_local_image], ((uint8_t*)src)+team_td->my_image*dist, nbytes);
  if(!(flags & GASNET_COLL_OUT_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
  return GASNET_COLL_INVALID_HANDLE;
}
...
#endif
  if(!(flags & GASNET_COLL_IN_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
  GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(((uint8_t*)dst)+team_td->my_image*dist, srclist[team_td->my_local_image], nbytes);
  if(!(flags & GASNET_COLL_OUT_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
  return GASNET_COLL_INVALID_HANDLE;
}
...
#endif
  if(!(flags & GASNET_COLL_IN_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
  for (dst = team_td->my_image+1; dst<team->my_images; dst++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[dst], nbytes, team_td->my_image),
                                        srclist[team_td->my_image], nbytes);
  }
  for (dst = 0; dst<=team_td->my_image; dst++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[dst], nbytes, team_td->my_image),
                                        srclist[team_td->my_image], nbytes);
  }
  if(!(flags & GASNET_COLL_OUT_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
...
#endif
  if(!(flags & GASNET_COLL_IN_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
  for (src = team_td->my_image+1; src<team->my_images; src++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[team_td->my_image], nbytes, src),
                                        srclist[src], nbytes);
  }
  for (src = 0; src<=team_td->my_image; src++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[team_td->my_image], nbytes, src),
                                        srclist[src], nbytes);
  }
  if(!(flags & GASNET_COLL_OUT_NOSYNC)) smp_coll_barrier(team_td->smp_coll_handle,0);
                                 ...
                                 GASNETE_THREAD_FARG) 
{
  ...
  for (dst = team_td->my_image+1; dst<team->my_images; dst++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[dst], nbytes, team_td->my_image),
                                        gasnete_coll_scale_ptr(srclist[team_td->my_image], nbytes, dst), nbytes);
  }
  for (dst = 0; dst<=team_td->my_image; dst++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[dst], nbytes, team_td->my_image),
                                        gasnete_coll_scale_ptr(srclist[team_td->my_image], nbytes, dst), nbytes);
                                 ...
                                 GASNETE_THREAD_FARG) 
{
  ...
  for (src = team_td->my_image+1; src<team->my_images; src++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[team_td->my_image], nbytes, src),
                                        gasnete_coll_scale_ptr(srclist[src], nbytes, team_td->my_image), nbytes);
  }
  for (src = 0; src<=team_td->my_image; src++) {
    GASNETE_FAST_UNALIGNED_MEMCPY_CHECK(gasnete_coll_scale_ptr(dstlist[team_td->my_image], nbytes, src),
                                        gasnete_coll_scale_ptr(srclist[src], nbytes, team_td->my_image), nbytes);
