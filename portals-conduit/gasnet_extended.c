...
static void gasnete_check_config() {
  ...
  gasneti_assert_always(gasnete_eopaddr_isnil(OPADDR_NIL));
}
...
gasnete_eop_t *gasnete_eop_new(gasnete_threaddata_t * const thread) {
  ...
  if_pt (!gasnete_opaddr_isnil(head)) {
    ...
    gasneti_assert(GASNETE_THREADID(eop) == thread->threadidx);
    ...
    { /* verify new free list got built correctly */
      ...
      for (i=0;i<(bufidx==255?255:256);i++) {                                   
        ...
        gasneti_assert(GASNETE_THREADID(eop) == threadidx);                  
...
gasnete_iop_t *gasnete_iop_new(gasnete_threaddata_t * const thread) {
  gasnete_opaddr_t head = thread->iop_free;
  if_pt (!thread->iop_free) {
    gasnete_iop_t *iop = thread->iop_free;
    thread->iop_free = iop->next;    
    iop->next = NULL;                
    gasneti_assert(GASNETE_THREADID(iop) == thread->threadidx);
    gasneti_assert(iop == IOP_ADDR_TO_PTR(GASNETE_THREADID(iop),iop->addr));
    gasneti_assert(OPTYPE(iop) == OPTYPE_IMPLICIT);
    gasneti_assert(OPSTATE(iop) == OPSTATE_FREE);
    SET_OPSTATE(iop, OPSTATE_INFLIGHT);
    ...
    thread->iop_num_bufs++;
    buf = (gasnete_iop_t *)gasneti_calloc(256,sizeof(gasneti_iop_t));
    ...
    for (i=0; i < 256; i++) {
      ...
      SET_OPSTATE(&(buf[i]),OPSTATE_FREE); 
      SET_OPTYPE(&(buf[i]),OPTYPE_IMPLICIT); 
    }
    ...
    { /* verify new free list got built correctly */
      ...
      while (p) {
	gasnete_threadidx_t p_th = GASNETE_THREADID(p);
	gasnete_iop_t *me;
	gasneti_assert(threadidx == p_th);
	me = IOP_ADDR_TO_PTR(p_th,p->addr);
	gasneti_assert(p == me);
...
int gasnete_op_isdone(gasnete_op_t *op) {
    gasneti_assert(GASNETE_THREADID(op) == gasnete_mythread()->threadidx);
...
void gasnete_op_free(gasnete_op_t *op) {
  gasnete_threaddata_t * const thread = gasnete_threadtable[GASNETE_THREADID(op)];
...
void gasnete_bb_init(size_t nchunks)
{
  ...
  gasneti_assert(sizeof(gasnete_bb_chunk_t) == GASNETE_BB_CHUNKSIZE);
  ...
  bb_md.user_ptr = (uint64_t)GASNETE_BB_MD;
  bb_md.eq_handle = gasnete_eq_h;

  /* register the md and get handle */
  GASNETE_PTLSAFE(PtlMDBind(geanete_ni_h, bb_md, PTL_RETAIN, &gasnete_bb_md_h));
}

int gasnete_bb_chunk_alloc(size_t nbytes, uintptr_t *offset)
{
...
void gasnete_portals_init(void)
{
  ...
  gasnete_bb_numchunk = gasneti_getenv_withdefault("GASNET_PORTAL_NUM_BB",GASNETE_BB_NUM_CHUNK);
  gasnete_max_tmpmd = gasneti_getenv_withdefault("GASNET_PORTAL_NUM_TMPMD",GASNETE_MAX_TMP_MDS);
  ...
  PTL_IFACE_DUP:
    break;
  default:
    gasneti_fatal("GASNet Portals failed on call to PtlNiInit:\n"
  ...
  GASNETE_PTLSAFE(PtlEQAlloc(gasnete_ni_h, eq_len, PTL_EQ_HANDLER_NONE, &gasnete_eq_h));
...
ptl_handle_md_t gasnete_alloc_tmpmd(void* dest, size_t nbytes)
{
   ...
   */
  while (gasneti_weakatomic_read(&gasnete_tmpmd_count,0) >= GASNETE_TMPMD_MAX) {
    GASNETI_SAFE(gasneti_AMPoll());
  }
  ...
  md.start = dest;
  md.length = nbytes;
  md.threashold = PTL_MD_THRESH_INF;
  md.max_size = 0;
  md.options = PTL_MD_EVENT_START_DISABLE;
  md.user_ptr = (uint64_t)GASNETE_TMP_MD;
  md.eq_handle = gasnete_eq_h;
...
void gasnete_poll(void)
{
   ...
   * are waiting.  Try it first.  If several exist, run them back to back since they
   * release resources.
   */
  while (processed < max_try) {
    ...
    default:
      gasneti_fatalerror("GASNet Portals Error in PtlEQGet: %s (%i)\n at %s\n",
			 ptl_err_src[rc],rc,gasneti_current_loc);
      break;
    }
    if (got_one) {
      gasnete_event_handler(&ev, (int)(uintptr_t)ev->md.userptr);
      processed++;
    } else {
 ...
 */
void gasnete_event_handler(ptl_event_t *ev, int which_md)
{
  ...
  case PTL_EVENT_SEND_END:
    ...
    gasneti_assert(msg_type & GASNETE_MSG_PUT);
    op = gasnete_opaddr_to_ptr(threadid, addr);
    if (msg_type & GASNETE_PTL_MSG_DOLC) {
      gasnetd_threaddata_t *th = gasnete_threadtable[GASNETE_THREADID(threadid)];
    ...
    if (which_md == GASNETE_BB_MD) {
      ...
      gasnete_chunk_free(offset);
    }
    break;
	
  case PTL_EVENT_ACK:
    /* A Put operation has completed remotely */
    gasnete_get_mbits_lowbits(match_bits, &threadid, &msg_type, &addr);
    gasneti_assert(msg_type & GASNETE_MSG_PUT);
    op = gasnete_opaddr_to_ptr(threadid, addr);
    ...
    gesnete_op_markdone(op, 0 /* !isget */);
    break;
	
  case PTL_EVENT_REPLY_END:
    /* extract the operation pointer from the match_bits */
    gasnete_get_mbits_lowbits(match_bits, &threadid, &msg_type, &addr);
    gasneti_assert(msg_type & GASNETE_MSG_GET);
    op = gasnete_opaddr_to_ptr(threadid, addr);
    ...
    if (which_md == GASNETE_BB_MD) {
      ...
      void **dest;
      *dest = *(uintptr_t*)q;
      memcpy(*dest,pdata,ev->len);
      /* free the bounce buffer */
      offset -= sizeof(void*);
      gasnete_bb_free(offset);
  ...
  default:
    ...
    gasneti_fatalerror("GASNet Portals Unexpected event in gasnete_event_handler:\n"
		       "  %s [%i]\n  at %s\n",prl_event_str[ev->type],ev->type,
		       gasneti_current_loc);
  }
...
extern gasnet_handle_t gasnete_get_nb_bulk (void *dest, gasnet_node_t node, void *src, size_t nbytes GASNETE_THREAD_FARG) {
  if (nbytes <= GASNETE_PTL_MAX_TRANS_SZ) {
    ...
    ptl_handle_md_t md_h;
    ptl_process_id_t target_id = gasnete_ptl_nodeid(dest);
    ptl_ac_index_t ac_index = GASNETE_PTL_AC_ID;
    ptl_match_bits_t match_bits = 0ULL;
    uint8_t lbits = GASNETE_RAR_MBITS | GASNETE_MSG_GET;
    /* encode gasnet handle into match bits, upper bits ignored */
    gasnete_set_mbits_lowbits(&match_bits, lbits, op);
    ...
    } else if ( (nbytes <= (GASNETE_BB_CHUNK_SIZE - (sizeof(void*))))  &&
...
gasnet_handle_t gasnete_put_nb_inner(gasnet_node_t node, void *dest, void *src, size_t nbytes, int isbulk GASNETE_THREAD_FARG) {
  if (nbytes <= GASNETE_PTL_MAX_TRANS_SZ) {
    ...
    ptl_ac_index_t ac_index = GASNETE_PTL_AC_ID;
    ptl_match_bits_t match_bits = 0ULL;
    uint8_t lbits = GASNETE_RAR_MBITS | GASNETE_MSG_PUT;
    ...
    } else if ( (nbytes <= GASNETE_BB_CHUNK_SIZE)  &&
		gasnete_bb_chunk_alloc(nbytes, &local_offset) ) {
      /* Encode dest addr in BB chunk for later copy */
      void* bb;
      md_h = gasnete_bb_md_h;
...
extern void gasnete_get_nbi_bulk (void *dest, gasnet_node_t node, void *src, size_t nbytes GASNETE_THREAD_FARG) {
  ...
  while (nbytes > 0) {
    ...
    if (gasnete_in_local_rar(dest,toget)) {
      md_h = gasnete_raram_md_h;
      local_offset = GASNETE_PTL_OFFSET(gasneti_mynode,dest);
    } else if ( (toget <= (GASNETE_BB_CHUNK_SIZE - (sizeof(void*))))  &&
...
void gasnete_put_nbi_inner(gasnet_node_t node, void *dest, void *src, size_t nbytes, int isbulk GASNETE_THREAD_FARG) {
  ...
  while (nbytes > 0) {
    ...
    } else if ( (toput <= GASNETE_BB_CHUNK_SIZE)  &&
...
extern int  gasnete_try_syncnbi_gets(GASNETE_THREAD_FARG_ALONE) {
  ...
  #endif
  {
    ...
    gasneti_assert(GASNETE_THREADID(iop) == mythread->threadidx);
    gasneti_assert(OPTYPE(iop) == OPTYPE_IMPLICIT);
    #if GASNET_DEBUG
      if (iop->next != NULL)
...
extern gasnet_register_value_t gasnete_wait_syncnb_valget(gasnet_valget_handle_t handle) {
  gasnet_register_value_t val;
  gasnete_threaddata_t * const thread = gasnete_threadtable[GASNETE_THREADID(handle)];
  gasneti_assert(thread == gasnete_mythread());
