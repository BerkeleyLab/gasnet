...
void gasnetc_init_sysv(gasneti_bootstrapExchangefn_t exchangefn) {
  ...
  if (exchangefn != NULL) {
     ...
     * communicate the prefix generated by the first in each supernode. */
    tmp = gasneti_malloc(gasneti_nodes * GASNETI_SYSV_PREFIX_LEN);
    (*exchangefn)(&gasneti_vnetname, GASNETI_SYSV_PREFIX_LEN, tmp);

    /* Non-first extract the shared prefix and (re)generate the gasneti_vnetname */
    if (gasneti_mysysvnode != 0) {
      gasneti_sysv_prefix = gasneti_strndup((const char *)(tmp + gasneti_firstsysvnode),
                                            GASNETI_SYSV_PREFIX_LEN);
      gasneti_sysv_makename(gasneti_sysvnodes, gasneti_vnetname);
    }
    gasneti_free(tmp);
