...
The current interface to the distributed scratch space management is based on each collective operation registering itself with the scratch space. When an operation registers itself it provides the $<$\texttt{tree geometry, root}$>$ tuple along with the size of the collective. The registration function returns the position of your local scratch space. Once a given operation registers itself with the scratch space it can then query the scratch space manager to find out the location to write of other peers. These positions and queries will mostly be done locally since we can calculate this information based on the tree geometry in use.
...
The current implementation registration of operations is given in Algorithm~\ref{registerscratchop} while the algorithm for getting the peer's position is given in Algorithm~\ref{getscratchpeerpos}.
...
\begin{algorithm}[h]
\begin{algorithmic}[1]
\STATE Input: A $<$\texttt{tree\_geometry\_in, root\_in}$>$ request pair along with a size $s$
\STATE Output: The location on local scratch space where to expect data for this operation
\STATE /* The barrier code has been modified to set a bit $barrier\_reset$ indicating that a barrier requests a reset of the scratch space whenever possible*/
\STATE $perform\_reset \leftarrow false$
\IF{this is the first collective after a barrier}
...
\ENDIF
\IF{$barrier\_reset$}
\STATE \texttt{PerformScratchReset()}
...
\STATE Send a reset message to any node who will send to my scratch space
\STATE $my\_scratch\_head\_pos \leftarrow s$
\STATE  $retval \leftarrow 0$
\ELSE
\STATE $retval \leftarrow my\_scratch\_head\_pos$
\STATE $my\_scratch\_head\_pos \leftarrow my\_scratch\_head\_pos + s$
\ENDIF
\STATE allocate a operation and attach it to the list of active operations
\STATE return $retval$

\end{algorithmic}
\caption{Register New Collective Operation }
\label{registerscratchop}
\end{algorithm}
