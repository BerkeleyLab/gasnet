...
\subsubsection{Data Structures} In order to enable the tree based data structures we need to maintain the tree topology information at each node. 
...
There are two versions of the tree that are possible to show. One where it is a ``local" view of the tree and another ``global" view of the tree. A ''local view" of the tree implies that each node will only know the data presented in Table~\ref{tab:treeinfo} and not care what the view is for the other nodes. In a ``global" view of the tree one can imagine that every node knows the full geometry of the tree. Each storage mechanisms has its advantages and disadvantages.

\textbf{INSERT MORE ON THE ADVANTAGES AND DISADVANTAGES}
...
The interface to the cache will be as follows: Each new collective call will request a local view of the tree that contains the information given in Table~\ref{tab:treeinfo} and can access the fields through accessor macros. The request will be of the form (tree type, fanout, root, team). Once the client has been issued a ``local" view of the tree geometry it is then responsible for freeing that local view through freeing routines provided by the cache. 

\noindent \textbf{Implementation Details}

Since we expect the number of trees (and dissemination orders) that any given application will use to be small we will encode this caching structure as a linked list however make it easy to replace it with a hash table once the time is right. Note that in the future this entire code of picking the correct tree will be replaced by the auto-tuning module so it needs to be able to be amenable to that mechanism. Initially this mechanism will be the simplest one can imagine. 
...
\subsection{Scratch Space Mangement}
...
Each node will first allocate a circular buffer that will be used for scratch space during the collective calls. At each barrier call we know that we can reset this circular buffer since the completion of the data movement will be enforced. The initial implementation will assume that only one type of tree geometry (i.e. a root id / tree type tuple) \footnote{and possibly collective communication?} will occur between barrier calls. If the user decides to relax this assumption a barrier will be inserted to enforce this guarantee (see Logging Section about how this will get relaxed later). 

Since each node assumes a fixed tree geometry between barrier calls the parent node can constantly update where the next available buffer slot in the children is. This also allows the parent to know when it is about to overun data it has sent to the children that has not yet been acknowledged. Thus when a parent sees that is about to overrun child's buffer space it has to wait for the child to signal the amount of scratch space that has been consumed. Similarly a child keeps track of the last time it has sent an ok-to-send ack to its parent and what the buffer spot is. Once a child realizes that it is approaching a limit about where the parent \textit{thinks} the last consumed piece of data is, it needs to send a message acknowledging the safe reuse of data slots and signaling the last piece of data that it is safe to overwrite. Thus If the data is getting consumed quickly this should not be a big issue. (think about how this propagates backwards). 

\noindent \textbf{Implementation Details}

Upon team creation each team will  need to provide a local buffer that will be used for the scratch space for that team. Having scratch space for different teams interfere adds unneeded complexity to the implementation. However for the primordial GASNET\_TEAM\_ALL we will carve up a segment of the \texttt{auxseg} and use that for the initial scratch space. 

Since the scratch space management will be on a per-team basis each team will need to keep track of the scratch positions of all the children so that each node knows where to send the data into the child'
s scratch space. However since there is a limited amount of buffer space available an ``out-of-band" mechanism also needs to be created to ensure that scratch space is reused. Upon a collective that requires an \texttt{OUT\_ALL\_SYNC} semantics we can use it to reset the buffers since it can be assumed that the collective call will not complete (and thus new collectives will not be issued) until the current collective is complete. When dealing with non-blocking collectives this implies that after the sync has returned (not the initiation) from the all sync collective the scratch space can be reset. 

We will also assume that the scratch space can be reset on a barrier. The main implementation problems that could arise are nonblocking collective calls initiated before a barrier and a sync taking place after taht given barrier. Thus to simplify the initial implementation we will assume that \textit{there will be no barriers betweeen a nonblocking collective initiation and a nonblocking collective sync}. This restriction could be lifted later. 
