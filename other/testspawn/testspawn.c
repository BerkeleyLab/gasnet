...
int main(int argc, char **argv) {
  gasnetex_rank_t *array1, *array2;
  gasnetex_rank_t i, j;

  gasneti_bootstrapInit(&argc, &argv, &gasneti_nodes, &gasneti_mynode);
  gasneti_bootstrapBarrier();
  array1 = gasneti_calloc(gasneti_nodes, sizeof(gasnetex_rank_t));
  array2 = gasneti_calloc(gasneti_nodes, sizeof(gasnetex_rank_t));
  ...
  for (i=0; i<gasneti_nodes; ++i) {
    gasneti_bootstrapBroadcast(&gasneti_mynode, sizeof(gasnetex_rank_t), &j, i);
    if (j != i) {
  ...
  gasneti_bootstrapExchange(&gasneti_mynode, sizeof(gasnetex_rank_t), array1);
  for (i=0; i<gasneti_nodes; ++i) {
    if (array1[i] != i) {
  ...
  gasneti_bootstrapAlltoall(array1, sizeof(gasnetex_rank_t), array2);
  for (i=0; i<gasneti_nodes; ++i) {
