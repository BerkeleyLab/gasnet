...
/* NOTES
   ...
   The following are needed to handle startup and termination:
      extern gasneti_spawnerfn_t const * gasneti_bootstrapInit_ssh(int *argc_p, char ***argv_p,
                                            gasnetex_rank_t *nodes_p,
                                            gasnetex_rank_t *mynode_p);
      void Fini(void);
...
static gasnetex_rank_t nranks = 0;
...
static struct child {
  ...
  gasnetex_rank_t       tree_ranks; /* ranks in this sub-tree, including self (1 for rank procs) */
  gasnetex_rank_t       tree_nodes; /* nodes in this sub-tree, including self (1 for rank procs) */
  char **             nodelist;
#if GASNET_BLCR
...
static gasnetex_rank_t out_degree = GASNETI_DEFAULT_SSH_OUT_DEGREE;
static struct fds {
  fd_set set;
  int    max;
} child_fds, all_fds;
static int parent = -1; /* socket */
static gasnetex_rank_t myrank = 0;
static int myname = -1;
static int children = 0;
static int ctrl_children = 0;
static gasnetex_rank_t tree_ranks = (gasnetex_rank_t)(-1L);
static gasnetex_rank_t tree_nodes = (gasnetex_rank_t)(-1L);
static int mypid;
static volatile int exit_status = 0;
static gasnetex_rank_t nnodes = 0;	/* nodes, as distinct from ranks */
...
/* BLCR-based checkpoint/restart */
  ...
  static int blcr_rollback(gasnetex_rank_t rank, const char *dir);
  static int blcr_restart(gasnetex_rank_t rank, const char *dir);
  static int blcr_reap(gasnetex_rank_t rank, int block);
...
static char ** short_nodelist(char **nodelist, gasnetex_rank_t count) {
  if (nnodes_set) {
...
static void send_identity(int s, struct child *ch) {
  ...
  iov[0].iov_len  = sizeof(gasnetex_rank_t);
  iov[1].iov_base = (void*) &ch->rank;
  iov[1].iov_len  = sizeof(gasnetex_rank_t);
  iov[2].iov_base = (void*) &ch->tree_ranks;
  iov[2].iov_len  = sizeof(gasnetex_rank_t);
  iov[3].iov_base = (void*) &ch->tree_nodes;
  iov[3].iov_len  = sizeof(gasnetex_rank_t);
  do_writev(s, iov, 4);
}
...
static void post_spawn(int count, int argc, char * const *argv) {
  ...
  while (count--) {
...
#endif
    do_read(s, &child_id, sizeof(gasnetex_rank_t));
    ...
    send_env(s);
    if (ch->tree_nodes > 1) {
      do_write(s, &out_degree, sizeof(gasnetex_rank_t));
...
static void do_connect(const char *spawn_args, int *argc_p, char ***argv_p)
{
  ...
  do_write(parent, &child_id, sizeof(gasnetex_rank_t));
...
static void spawn_one_control(gasnetex_rank_t child_id, const char *cmdline, const char *my_host) {
...
static void cmd_EXCHG(char cmd, int i) {
  ...
  if (cmd == cmd0) {
    ...
    gasnetex_rank_t next = myrank + tree_ranks;
    iov[0].iov_base = data;
...
static void cmd_SNBCAST(char cmd, int i) {
  ...
  /* State: */
  static uint8_t *data = NULL;
  static gasnetex_rank_t *roots = NULL;
  static struct iovec *iov = NULL;
  static int count = 0;
  static size_t len;
  ...
  if (cmd == cmd0) {
    ...
    if (!count) {
      ...
      roots = gasneti_malloc(tree_ranks * sizeof(gasnetex_rank_t));
      if (! is_root) {
    ...
    {
      gasnetex_rank_t offset = (child[i].rank - myrank);
      gasnetex_rank_t *r = roots + offset;
      int j, root_count;
      do_read(s, r, child[i].tree_ranks * sizeof(gasnetex_rank_t));
    ...
    if (is_root) {
      ...
      iov[2].iov_len  = tree_ranks * sizeof(gasnetex_rank_t);
...
static gasnetex_rank_t nodediv_alloc(nodediv_t *x, gasnetex_rank_t n) {
  gasnetex_rank_t rem = MIN(x->rem, n);
  x->rem -= rem;
  return n * x->quot + rem;
}
...
static void do_common(int argc, char **argv)
{
  ...
  /* Layout children: */
  {
    ...
    sublist = nodelist;
    for (j = 0; j < ctrl_children; ++j) {
      const gasnetex_rank_t nodes = nodediv_alloc(&npc, 1);
      const gasnetex_rank_t procs = nodediv_alloc(&ppn, nodes);
      child[j].rank       = rank;
...
static void do_master(const char *spawn_args, int *argc_p, char ***argv_p) {
  ...
  if ((lnproc < 1) || ((long)nranks != lnproc)) { /* Non-positive or Overflow */
    die(1, "Process count %ld is out-of-range of gasnetex_rank_t", lnproc);
  }
  ...
  if (nnodes_set) {
    ...
    if ((lnnodes < 1) || ((long)nnodes != lnnodes)) { /* Non-positive or Overflow */
      die(1, "Node count %ld is out-of-range of gasnetex_rank_t", lnnodes);
    }
    if (nnodes > nranks) {
 ...
 * with the spawning.
 */
extern gasneti_spawnerfn_t const * gasneti_bootstrapInit_ssh(int *argc_p, char ***argv_p, gasnetex_rank_t *nodes_p, gasnetex_rank_t *mynode_p) {
...
static void bootstrapExchange(void *src, size_t len, void *dest) {
  ...
  const gasnetex_rank_t next = myrank + 1;
  struct iovec iov[3];
...
static void bootstrapSNodeBroadcast(void *src, size_t len, void *dest, int rootnode_arg) {
  ...
  const gasnetex_rank_t rootnode = rootnode_arg;
  struct iovec iov[4];
...
static int blcr_rstart_request(gasnetex_rank_t rank, const char *dir, char rstrt_args) {
