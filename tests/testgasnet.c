...
void doit3(int partner, int *partnerseg) {
  ...
  { /*  value test */
    ...
    for (i=0; i < 100; i++) {
      gex_Event_Wait(gasnetex_put_nb_val(myteam, partner, partnerseg+i+100, 1000 + mynode + i, sizeof(int), 0));
    }
    for (i=0; i < 100; i++) {
    ...
    for (i=0; i < 100; i++) {
      gex_Event_Wait(gasnetex_put_nb_val(myteam, partner, partnerbase2+i+100, 100 + mynode + i, sizeof(unsigned char), 0));
    }
    for (i=0; i < 100; i++) {
  ...
  { GASNET_BEGIN_FUNCTION();
    ...
    for (i = 0; i < MAX(1,iters/10); i++) {
      ...
      for (sz = 1; sz <= MAXSZ; sz*=2) {
        ...
        gex_Event_Wait(gasnetex_get_nb(myteam, localpos, partner, rsegpos, sz, 0));
        gex_Event_Wait(gasnetex_get_nb(myteam, localpos+elems, partner, rsegpos+elems, sz, 0));
        gex_Event_Wait(gasnetex_get_nb(myteam, segpos, partner, rsegpos+2*elems, sz, 0));
        gex_Event_Wait(gasnetex_get_nb(myteam, segpos+elems, partner, rsegpos+3*elems, sz, 0));
