...
void ping_longhandler(gasnetex_token_t token, void *buf, size_t nbytes) {
    ...
    gasnetex_AMReplyLong0(token, hidx_pong_longhandler, rand_payload, nbytes, peerseg + LSZ(0), GASNETEX_LC_INIT, 0);
}
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < iters; ++i) {
    ...
    gasnetex_AMRequestMedium0(myteam, peer, hidx_ping_medhandler, rand_payload, MSZ(0), GASNETEX_LC_INIT, 0);
    GASNET_BLOCKUNTIL(flag == goal); ++goal;

    gasnetex_AMRequestLong0(myteam, peer, hidx_ping_longhandler, rand_payload, LSZ(0), peerseg, GASNETEX_LC_INIT, 0);
    GASNET_BLOCKUNTIL(flag == goal); ++goal;
  }
