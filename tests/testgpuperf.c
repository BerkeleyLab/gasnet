/* $Source: /Users/kamil/work/gasnet-cvs2/gasnet/tests/Attic/testgpuperf.c,v $
 * $Date: 2010/07/15 20:25:23 $
 * $Revision: 1.1.2.1 $
 *
...
#define TEST_SEGSZ_EXPR 1024*1024*1024 
...
void test_put_host2host(size_t nelems, int iters)
{
  gasnet_handle_t h;
  int i, expected_val;
  size_t nbytes;
  ...
  if (mynode == 0) {    
    ...
    for (i=0; i<iters; i++) {
      ...
      h = gasnet_put_nb(dstNode, hostB_ptrs[dstNode], hostA, nbytes 
                        GASNETE_THREAD_PASS);
      gasnet_wait_syncnb(h);
    }
    total_time = TIME() - start_time;
    printf("put_host2host: %lu nbytes, iters %d, time %lg (us), bw %lg (MB/s)\n",
           nbytes, iters, (double)total_time/iters, 
           (double)nbytes/((double)total_time/iters));
  }
  BARRIER();  
}
...
/* Assume only 2 gasnet processes are running */
void test_put_host2gpu(size_t nelems, int iters)
{
  gasnet_handle_t h;
  int i, expected_val;
  size_t nbytes;
  ...
  if (mynode == 0) {    
    ...
    total_time = TIME() - start_time;
    printf("put_host2gpu: %lu nbytes, iters %d, time %lg (us), bw %lg (MB/s)\n",
           nbytes, iters, (double)total_time/iters, 
           (double)nbytes/((double)total_time/iters));
  }
  BARRIER();  
...
/* Assume only 2 gasnet processes are running */
void test_put_gpu2host(size_t nelems, int iters)
{
  ...
  if (mynode == 0) {    
    ...
    total_time = TIME() - start_time;
    printf("put_gpu2host: %lu nbytes, iters %d, time %lg (us), bw %lg (MB/s)\n",
           nbytes, iters, (double)total_time/iters, 
           (double)nbytes/((double)total_time/iters));
  }
  BARRIER();  
...
void test_put_gpu2gpu(size_t nelems, int iters)
{
  gasnet_handle_t h;
  int i, expected_val;
  size_t nbytes;
  ...
  if (mynode == 0) {    
    ...
    total_time = TIME() - start_time;
    printf("put_gpu2gpu: %lu nbytes, iters %d, time %lg (us), bw %lg (MB/s)\n",
           nbytes, iters, (double)total_time/iters, 
           (double)nbytes/((double)total_time/iters));
  }
  BARRIER();  
...
int main(int argc, char **argv)
{
  ...
  for (nelems=1; nelems<=MAX_DATA_SIZE/sizeof(int); nelems *= 2) {
    if (nelems < 1024*1024)
      ...
      iters = 20;
    test_put_host2host(nelems, iters);
  }
  ...
  for (nelems=1; nelems<=MAX_DATA_SIZE/sizeof(int); nelems *= 2) {
    if (nelems < 1024*1024)
      ...
      iters = 20;
    test_put_host2gpu(nelems, iters);
  }
  ...
  for (nelems=1; nelems<=MAX_DATA_SIZE/sizeof(int); nelems *= 2) {
    if (nelems < 1024*1024)
      ...
      iters = 20;
    test_put_gpu2host(nelems, iters);
  }
