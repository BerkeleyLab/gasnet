                           ...
                           gasnet_handle_t handle_array[] GASNETE_THREAD_FARG)
{
    ...
    #if 0
    {
      /* Call the version that performs logging and error checking of args */
    ...
    #else
      for (i = 0; i < count; ++i) {
	handle_array[i] = (gasnet_handle_t)gasnete_eop_new(GASNETE_MYTHREAD);
      }
      gasnetc_rdma_get_X(count, node, src, dest, nbytes_array, handle_array);
    #endif
}
