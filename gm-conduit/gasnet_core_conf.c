/* $Id: gasnet_core_conf.c,v 1.5.2.2 2003/09/07 09:40:03 csbell Exp $
...
gasnetc_getconf_sockets(void)
{
	...
	if ((sscanf (id, "%hu", (unsigned short *) &gasnetc_mynode) != 1) && 
		...
		if (rank != NULL && *rank != '\0')
			gasnetc_mynode = (gasnet_node_t) atoi(rank);
		else {
			rank = getenv("LAMRANK");
			if (rank != NULL && *rank != '\0')
				gasnetc_mynode = (gasnet_node_t) atoi(rank);
