...
/* For SYSV, we need to have real file
 * created on the disk before calling ftok.
 * For that reason we need to call mkstemp()
 * multiple times (we cannot just create one
 * unique file name, and then derive the rest 
 * of them).
 * 
 * For POSIX, the files are created when 
 * calling open(). */

#if GASNET_SYSV
    #define UNIQUE_FILES 2
#else
    #define UNIQUE_FILES 1
#endif

    char *exchg[UNIQUE_FILES];
    char unique[UNIQUE_FILES][GASNETI_PSHM_UNIQUE_LEN];
    ...
    if (gasneti_pshm_mynode == 0) {
      for (i = 0; i < UNIQUE_FILES; i++){
        const char *tmp = gasneti_pshm_makenames(NULL, i);
        memcpy(unique[i], tmp, GASNETI_PSHM_UNIQUE_LEN);
      }
    }
    ...
    for (i = 0; i < UNIQUE_FILES; i++){
      exchg[i] = gasneti_malloc(gasneti_nodes * GASNETI_PSHM_UNIQUE_LEN);
      (*exchangefn)(unique[i], GASNETI_PSHM_UNIQUE_LEN, exchg[i]);
    }

    /* Non-first nodes now generate the same names from the unique identifier */
    if (gasneti_pshm_mynode != 0) {
      for (i = 0; i < UNIQUE_FILES; i++){
        (void)gasneti_pshm_makenames((const char *)(exchg[i] + gasneti_pshm_firstnode), i);
      }
    }
    for (i = 0; i < UNIQUE_FILES; i++){
      gasneti_free(exchg[i]);
    }
  }
