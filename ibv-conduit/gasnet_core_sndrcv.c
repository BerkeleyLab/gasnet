...
void gasnetc_rcv_am(const VAPI_wc_desc_t *comp, gasnetc_rbuf_t **spare_p) {
  ...
  if (GASNETC_MSG_ISREPLY(flags)) {
    ...
    if_pf (rbuf->rbuf_needReply) {
      ...
      uint32_t old;
      do {
	old = gasneti_weakatomicX_read(&cep->am_unsent, 0);
	if (old >= gasnetc_am_credits_slack) {
      ...
      } while (!gasneti_weakatomicX_compare_and_swap(&cep->am_unsent, old, old+1, 0));
    }
    if_pf (!spare) {
...
int gasnetc_ReqRepGeneric(gasnetc_category_t category, gasnetc_rbuf_t *token,
  ...
  if_pt (dest == gasneti_mynode) {
     ...
     */
    credits = gasneti_weakatomicX_read(&cep->am_unsent, 0);
    if (credits) {
      ++numargs;
    }
    ...
    if_pf (credits) {
      ...
      do { /* This loop is an unconditional atomic swap with zero. */
        tmp = gasneti_weakatomicX_read(&cep->am_unsent, 0);
      } while (tmp && !gasneti_weakatomicX_compare_and_swap(&cep->am_unsent, tmp, 0, 0));
      args[0] = tmp;
      i = 1;
...
int gasnetc_sreq_is_ready(gasnetc_sreq_t *sreq) {
  gasneti_sync_writes();
  return gasneti_weakatomicX_decrement_and_test(&(sreq->fh_ready), GASNETI_ATOMIC_REL);
}
...
int gasnetc_fh_put_helper(gasnet_node_t node, gasnetc_sreq_t *sreq,
  ...
  if_pt (fh_rem != NULL) {
    ...
    gasneti_weakatomicX_set(&sreq->fh_ready, 2, 0);
...
extern void gasnetc_sndrcv_init_peer(gasnet_node_t node) {
  ...
  if (node != gasneti_mynode) {
    for (i = 0; i < gasnetc_num_qps; ++i, ++cep) {
      ...
      gasneti_weakatomicX_set(&cep->am_unsent, 0, 0);
...
extern void gasnetc_counter_wait_aux(gasnetc_counter_t *counter, int handler_context)
{
  ...
  if (handler_context) {
    do {
      ...
      GASNETI_WAITHOOK();
      gasnetc_poll_snd();
    } while (initiated != gasneti_weakatomicX_read(&(counter->completed), 0));
  } else {
    do {
      GASNETI_WAITHOOK();
      gasnetc_poll_both();
