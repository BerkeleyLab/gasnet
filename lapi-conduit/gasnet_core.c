...
void gasnetc_lapi_rcallback(lapi_handle_t *hndl, void *sinfo, int *src)
{
	 ...
	 * particular operation.  Note that the tag is passed in sinfo
...
#if 0
	GASNETC_LCHECK(
	  LAPI_Put(*hndl, *src, sizeof(int), remote_address,
	  &gasnetc_lapi_done, NULL, NULL, NULL));
#else
    GASNETC_LCHECK(
      LAPI_Rmw(*hndl, SWAP, *src, remote_address,
                &gasnetc_lapi_done, NULL, NULL));
#endif
}
